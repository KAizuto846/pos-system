...
async function flushProductBatch(
  ...
  for (const data of batch) {
    ...
    if (existingRow) {
      try {
        const detectedUpdate = detectPiecesFromName(data.name);
        await prisma.product.update({
          ...
          data: {
            ...
            // Caja CT/(N): el producto se marca con su N para que recuerde
            // cuántas piezas trae (la pieza se crea si aún no existe).
            ...(detectedUpdate ? { piecesPerUnit: detectedUpdate.pieces, piecesTracked: true } : {}),
          },
        });

        // Si es una caja sin producto pieza, crearlo (una sola vez)
        if (detectedUpdate) {
          ...
          if (box) {
            const piece = await ensurePieceForBox(prisma, box, detectedUpdate.pieces);
            if (piece) {
              void logAudit({
                userId: 0,
                ...
                entity: 'product',
                entityId: piece.id,
                description: `Producto pieza creado automáticamente (importación) para la caja: ${box.name}`,
                after: { name: piece.name, barcode: piece.barcode, piecesPerUnit: detectedUpdate.pieces, pieceOfProductId: box.id },
              });
            }
          }
        }
...
async function flushCreateBatch(
  ...
  // Los productos cuyo nombre termina en CT/(N) son cajas: se crean con su N
  // y luego generan su producto pieza ("Pieza de ...", código S+).
  const boxNames = new Map<string, number>();
  const batchWithBoxes = batch.map(b => {
    const detected = detectPiecesFromName(String(b.name || ''));
    if (detected) {
      boxNames.set(b.name, detected.pieces);
      return { ...b, piecesPerUnit: detected.pieces, piecesTracked: true };
    }
    return b;
  });
  try {
    const inserted = await prisma.product.createMany({ data: batchWithBoxes });
    results.imported += inserted.count;

    // Crear productLines primarias para los productos nuevos con proveedor
    const withSupplier = batchWithBoxes.filter(b => b.supplierId);
    ...
    // Cajas CT/(N): crear el producto pieza de cada una (una sola vez)
    if (boxNames.size > 0) {
      const createdBoxes = await prisma.product.findMany({
        where: { name: { in: Array.from(boxNames.keys()) } },
        select: { id: true, name: true, barcode: true, price: true, cost: true, departmentId: true, supplierId: true },
      });
      for (const box of createdBoxes) {
        const pieces = boxNames.get(box.name);
        if (!pieces) continue;
        const piece = await ensurePieceForBox(prisma, box, pieces);
        if (piece) {
          void logAudit({
            userId: 0,
            userName: 'importación',
            ...
            entityId: piece.id,
            description: `Producto pieza creado automáticamente (importación) para la caja: ${box.name}`,
            after: { name: piece.name, barcode: piece.barcode, piecesPerUnit: pieces, pieceOfProductId: box.id },
          });
        }
      }
    }
