...
export async function GET() {
  try {
    ...
    const expiringToday = await prisma.stockBatch.findMany({
      where: {
        expiryDate: {
          not: null,
    ...
    const expiredBatches = await prisma.stockBatch.findMany({
      where: {
        expiryDate: { not: null, lt: new Date(now.getFullYear(), now.getMonth(), now.getDate()) },
        quantity: { gt: 0 },
      },
      include: { product: { select: { id: true, name: true, barcode: true, stock: true } } },
      orderBy: { expiryDate: "asc" },
    ...
    function groupByProduct(batches: typeof nearExpiryBatches) {
      ...
      for (const b of batches) {
        ...
        if (!entry.nearestExpiry || b.expiryDate < entry.nearestExpiry) {
          entry.nearestExpiry = b.expiryDate;
        }
      }
