...
interface ReceiveItem {
  ...
  price?: number | null;
  // Código de barras corregido en la ventana de recepción: muchas veces es el
  // mismo producto pero el proveedor actualizó el código.
  barcode?: string | null;
  ...
  try {
    ...
    const updatedOrder = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      for (const item of items) {
        ...
        // Código de barras corregido en la recepción: se actualiza el producto
        // si llegó uno distinto (mismo producto, código nuevo del proveedor).
        if (orderItem.productId && typeof item.barcode === "string") {
          const newBarcode = item.barcode.trim();
          const currentBarcode = orderItem.product?.barcode ?? "";
          if (newBarcode && newBarcode !== currentBarcode) {
            const taken = await tx.product.findFirst({
              where: { barcode: newBarcode, id: { not: orderItem.productId } },
              select: { name: true },
            });
            if (taken) {
              throw new Error(
                `El código "${newBarcode}" ya lo usa el producto "${taken.name}"`
              );
            }
            await tx.product.update({
              where: { id: orderItem.productId },
            ...
            barcodeChanges.push({
              ...
              productName: orderItem.product?.name || orderItem.productName || `#${orderItem.productId}`,
              antes: currentBarcode,
              despues: newBarcode,
            });
            void logChange(getDeviceId(), "UPDATE", "product", orderItem.productId, {
              barcode: newBarcode,
            });
          }
        }
        ...
        if (receivedQuantity > 0) {
          ...
          if (!productId) {
            ...
            // Si el nombre termina en CT/(N), es una caja: la marcamos con su N
            // y le creamos su producto pieza ("Pieza de ...", código S+).
            const detectedBox = detectPiecesFromName(name);
            const created = await tx.product.create({
              data: {
                ...
                active: true,
                ...(detectedBox
                  ? { piecesPerUnit: detectedBox.pieces, piecesTracked: true }
                  : {}),
              },
            });
            void logChange(getDeviceId(), "CREATE", "product", created.id, {
              ...
              minStock: 1,
              active: true,
              ...(detectedBox ? { piecesPerUnit: detectedBox.pieces, piecesTracked: true } : {}),
            });
            ...
            if (detectedBox) {
              ...
              if (piece) {
                void logChange(getDeviceId(), "CREATE", "product", piece.id, {
                  ...
                  piecesPerUnit: detectedBox.pieces,
                  pieceOfProductId: created.id,
                });
              }
            }
            await tx.supplierOrderItem.update({
        ...
        if (!productId) {
          ...
          const ghostCost =
            typeof extra.costPrice === "number" && extra.costPrice >= 0
              ? extra.costPrice
              : 0;
          const detectedExtraBox = detectPiecesFromName(name);
          const created = await tx.product.create({
            data: {
              name,
              barcode,
              ...
              active: true,
              ...(detectedExtraBox
                ? { piecesPerUnit: detectedExtraBox.pieces, piecesTracked: true }
                : {}),
            },
          });
          void logChange(getDeviceId(), "CREATE", "product", created.id, {
            ...
            minStock: 1,
            active: true,
            ...(detectedExtraBox ? { piecesPerUnit: detectedExtraBox.pieces, piecesTracked: true } : {}),
          });
          await tx.productLine.create({
            data: {
              productId: created.id,
    ...
    void logAudit({
      ...
      details: {
        ...
        status: updatedOrder.status,
        // Detalle por item: cuántas piezas entraron y con qué costo/caducidad
        recibidos: items.map((i) => {
          const oi = order.items.find((x) => x.id === i.orderItemId);
          const unitMultiplier = oi?.isBox && oi.unitsPerBox ? oi.unitsPerBox : 1;
          return {
            producto: oi?.product?.name || oi?.productName || `#${oi?.productId ?? '?'}`,
            cajasRecibidas: i.receivedQuantity,
            piezasEntrantes: Math.max(0, (i.receivedQuantity - (oi?.receivedQuantity ?? 0)) * unitMultiplier),
            ...(i.costPrice != null ? { costo: i.costPrice } : {}),
            ...(i.price != null ? { precioVenta: i.price } : {}),
            ...(i.expiresAt ? { caducidad: i.expiresAt } : {}),
            ...(i.finalStock != null ? { stockFinal: i.finalStock } : {}),
          };
        }),
        ...(barcodeChanges.length > 0 ? { codigosBarrasActualizados: barcodeChanges } : {}),
        ...(extras.length > 0 ? { extras: extras.map((e) => ({ nombre: e.name, cantidad: e.quantity, costo: e.costPrice })) } : {}),
        ...(totalNote !== null ? { totalNota: totalNote } : {}),
      },
      after: { status: updatedOrder.status, barcodeChanges },
    ...
    if (
      ...
      message.includes("ya lo usa") ||
      message.includes("debe tener")
    ) {
