...
export async function POST(request: Request) {
  try {
    ...
    void logAudit({
      ...
      details: {
        proveedor: order.newOrder.supplier?.name || `#${data.supplierId}`,
        estado: order.newOrder.status,
        notas: data.notes || null,
        productos: order.newOrder.items.map((i) => ({
          producto: i.product?.name || i.productName || 'sin inventario',
          codigo: i.product?.barcode || i.productBarcode || null,
          cantidad: i.quantity,
          ...(i.isBox ? { enCajas: true, piezasPorCaja: i.unitsPerBox } : {}),
          costoProveedor: i.costPrice,
        })),
      },
      after: {
        supplierId: data.supplierId,
