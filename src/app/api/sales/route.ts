...
export async function POST(request: Request) {
  try {
    ...
    const sale = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      for (const item of data.items) {
        ...
        if (result === 0 && Number.isInteger(item.productId)) {
          // Pieza con stock agotado: abrir cajas automáticamente (una por una)
          // hasta cubrir la venta o quedarse sin cajas. La pieza recuerda su N
          // (piecesPerUnit) incluso si la caja cambia.
          ...
          if (product?.pieceOfProductId) {
            // Límite de seguridad: nunca más de 100 cajas por renglón.
            for (let guard = 0; guard < 100 && result === 0; guard++) {
              const box = await tx.product.findUnique({
                where: { id: product.pieceOfProductId },
                select: { id: true, stock: true, piecesPerUnit: true },
              });
              if (!box || box.stock < 1) break;

              // Prioridad: N de la caja; si no tiene, el N que la pieza
              // recuerda desde su creación.
              const pieces = box.piecesPerUnit ?? product.piecesPerUnit ?? 0;
              if (pieces < 1) break;
              ...
              // Reintentar el descuento con el stock recién abierto
    ...
    void logAudit({
      ...
      details: {
        total: sale.total,
        descuento: data.discountTotal || 0,
        ...(data.cashReceived != null ? { recibido: data.cashReceived, cambio: data.change } : {}),
        ...(data.customerId ? { clienteId: data.customerId } : {}),
        productos: sale.items.map((i) => ({
          producto: i.product.name,
          codigo: i.product.barcode || null,
          cantidad: i.quantity,
          precio: i.price,
          stockRestante: i.product.stock,
        })),
        ...(openedPieces.length > 0
          ? { cajasAbiertas: openedPieces.map((o) => ({ cajaId: o.boxId, piezas: o.pieces })) }
          : {}),
        ...(stockShortages.length > 0
          ? { cobrosSinStock: stockShortages.map((s) => ({ producto: s.productName, stockAntes: s.stockBefore, stockDespues: s.stockAfter, faltante: s.shortage })) }
          : {}),
      },
      after: { total: sale.total, items: sale.items.map((i) => ({ producto: i.product.name, cantidad: i.quantity, precio: i.price })) },
