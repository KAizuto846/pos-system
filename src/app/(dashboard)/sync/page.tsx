  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-fg">Sincronizacion</h1>
          <p className="text-fg-muted mt-1">
            Conecta tus equipos para que compartan inventario, ventas y productos en tiempo real.
          </p>
        </div>
        <Badge variant="outline" className={cn('self-start sm:self-auto', isDesktop ? 'text-brand border-brand/50' : 'text-sky-400 border-sky-500/50')}>
      ...
      <Card className="border-line bg-surface-2">
        <CardHeader>
          <CardTitle className="text-lg text-fg flex items-center gap-2">
            <MonitorDot className="h-5 w-5 text-brand" />
            Mi equipo
          </CardTitle>
          <CardDescription>
            {isDesktop
        ...
        <CardContent className="grid gap-4 md:grid-cols-3">
          <div className="rounded-md bg-surface/60 border border-line px-4 py-3">
            <p className="text-xs text-fg-subtle">Nombre del equipo</p>
            <p className="text-sm text-fg font-medium mt-1">{deviceInfo.deviceName || '—'}</p>
          </div>
          <div className="rounded-md bg-surface/60 border border-line px-4 py-3">
            <p className="text-xs text-fg-subtle">Tu direccion local (compartela con tus equipos)</p>
            <div className="flex items-center gap-2 mt-1">
              <p className="text-sm text-fg font-mono truncate">{selfUrl}</p>
              <button type="button" onClick={handleCopyUrl} className="text-fg-muted hover:text-fg transition-colors shrink-0" title="Copiar URL">
                <Copy className="h-3.5 w-3.5" />
              </button>
            </div>
          </div>
          <div className="rounded-md bg-surface/60 border border-line px-4 py-3">
            <p className="text-xs text-fg-subtle">Servidor local</p>
            <p className="text-sm text-fg font-medium mt-1 flex items-center gap-1.5">
      ...
      <Card className="border-line bg-surface-2">
        ...
        <CardContent className="space-y-3">
          {tsState.available === undefined ? (
            <div className="flex items-center gap-2 text-sm text-fg-muted"><Loader2 className="h-4 w-4 animate-spin" />Verificando...</div>
          ) : !tsState.available ? (
            ...
            <>
              <div className="flex items-center gap-2">
                {tsState.online ? (
                  <Badge className="bg-brand-strong text-emerald-50"><CheckCircle2 className="h-3 w-3 mr-1" />Conectado a la red remota</Badge>
                ) : (
              ...
              {tsState.ip && (
                <div className="rounded-md bg-surface/60 border border-line px-4 py-3">
                  <p className="text-xs text-fg-subtle">IP privada (acceso solo entre tus equipos)</p>
                  <div className="flex items-center gap-2 mt-1">
                    <p className="text-sm text-fg font-mono truncate">http://{tsState.ip}:{deviceInfo.serverPort || 3000}</p>
                    <button type="button" onClick={() => { if (tsState.ip) void navigator.clipboard?.writeText(`http://${tsState.ip}:${deviceInfo.serverPort || 3000}`); }} className="text-fg-muted hover:text-fg transition-colors shrink-0" title="Copiar URL privada">
                      <Copy className="h-3.5 w-3.5" />
                    </button>
                  </div>
                </div>
              ...
              {tsState.dnsName && (
                <div className="rounded-md bg-surface/60 border border-line px-4 py-3">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <button type="button" onClick={() => { if (tsState.dnsName) void navigator.clipboard?.writeText(`https://${tsState.dnsName}/`); }} className="text-fg-muted hover:text-fg transition-colors shrink-0" title="Copiar URL">
                      <Copy className="h-3.5 w-3.5" />
                    </button>
                  </div>
      ...
      <Card className="border-line bg-surface-2">
        ...
        <CardContent className="space-y-4">
          {/* Paso 1 */}
          <div className="flex items-start gap-3 rounded-md border border-line bg-surface/40 px-4 py-3">
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-brand-strong/20 text-brand text-sm font-bold shrink-0 mt-0.5">1</div>
            <div className="min-w-0">
              <p className="text-sm font-medium text-fg">Conecta todos los equipos a la misma red</p>
            ...
            <CheckCircle2 className="h-4 w-4 text-brand shrink-0 mt-1 ml-auto" />
          </div>

          {/* Paso 2 */}
          <div className="flex items-start gap-3 rounded-md border border-line bg-surface/40 px-4 py-3">
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-brand-strong/20 text-brand text-sm font-bold shrink-0 mt-0.5">2</div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-fg">Permite la app en el Firewall de Windows</p>
              <p className="text-xs text-fg-muted mt-0.5">
              ...
              <div className="flex flex-wrap gap-2 mt-2">
                ...
                {!isWindows && (
                  <span className="text-[11px] text-fg-subtle self-center">(los botones se activan en la app de Windows)</span>
                )}
              </div>
            </div>
            {isWindows && <CheckCircle2 className="h-4 w-4 text-brand shrink-0 mt-1 ml-auto" />}
          </div>

          {/* Paso 3 */}
          <div className="flex items-start gap-3 rounded-md border border-line bg-surface/40 px-4 py-3">
            <div className="flex h-7 w-7 items-center justify-center rounded-full bg-brand-strong/20 text-brand text-sm font-bold shrink-0 mt-0.5">3</div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                <div>
                  <p className="text-sm font-medium text-fg">Busca los equipos detectados</p>
                  <p className="text-xs text-fg-muted mt-0.5">
                    {peers.length > 0
                  ...
                  <p className="text-[11px] text-fg-subtle mt-0.5">
                    Con el botón <span className="text-fg-muted">Copiar BD</span> reemplazas TODA la información local por la del equipo seleccionado (como una copia de seguridad en vivo). Despues sigue sincronizando solo en tiempo real.
                  </p>
                </div>
              ...
              {peers.length > 0 && (
                <ul className="mt-3 space-y-2">
                  {peers.map((peer) => (
                    <li key={`${peer.ip}:${peer.port}`} className="flex items-center justify-between gap-3 rounded-md bg-surface-2/80 border border-line px-3 py-2">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-line text-fg-muted flex-shrink-0">
                          <Server className="h-4 w-4" />
                        </div>
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-fg truncate">{peer.name || 'Equipo'}</p>
                        <p className="text-xs text-fg-muted font-mono">{peer.ip}:{peer.port}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      <Badge variant="outline" className="text-brand border-brand/50 flex-shrink-0">
              ...
              {peers.length === 0 && (
                <div className="mt-3 rounded-md bg-canvas/50 border border-surface-2 px-4 py-3 flex items-center gap-3">
                  <Wifi className="h-4 w-4 text-fg-subtle shrink-0" />
                  <p className="text-xs text-fg-subtle">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="border-line bg-surface-2 lg:col-span-1">
          <CardHeader>
            <CardTitle className="text-lg text-fg flex items-center gap-2">
          ...
          <CardContent className="space-y-3">
            ...
            {lastResult && (
              <div className="rounded-md bg-line/40 px-3 py-2 space-y-1.5">
                <div className="flex justify-between text-xs">
                  <span className="text-fg-muted">Ultima sincronizacion LAN:</span>
                  <span className="text-fg">{new Date(lastResult.at).toLocaleTimeString()}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-fg-muted">Equipos contactados:</span>
                  <span className={cn('font-medium', lastResult.peers > 0 ? 'text-brand' : 'text-fg-muted')}>
                ...
                {lastResult.results.slice(0, 4).map((r) => (
                  <div key={r.peer} className="flex items-center justify-between gap-2">
                    <span className="text-xs text-fg-muted font-mono truncate">{r.peer}</span>
                    {r.ok ? (
            ...
            <div className="rounded-md bg-surface/60 border border-line px-3 py-2 space-y-1">
              <p className="text-xs text-fg-subtle">Registro de cambios</p>
              <div className="flex justify-between text-xs">
                <span className="text-fg-muted">Cambios totales:</span>
                <span className="text-fg font-mono">{stats?.totalChanges ?? '—'}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-fg-muted">Pendientes:</span>
                <span className={cn('font-mono', (stats?.pendingSync ?? 0) > 0 ? 'text-amber-400' : 'text-brand')}>
                  {stats?.pendingSync ?? '—'}
                </span>
              </div>
            </div>
        ...
        <Card className="border-line bg-surface-2 lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-lg text-fg flex items-center gap-2">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {relayState.relayUrl && (
                <Badge variant="outline" className={cn(relayState.connected ? 'text-brand border-brand/50' : 'text-red-400 border-red-500/50')}>
            ...
            {relayLastSync && (
              <div className="rounded-md bg-line/50 px-4 py-3 text-sm text-fg-muted">
            ...
            <div className="rounded-md bg-sky-950/30 border border-sky-800/50 px-4 py-3 text-xs text-fg-muted">
        ...
        <CardContent className="space-y-3 text-sm text-fg-muted">
          ...
          <GuideSection title="2. Equipos en redes distintas (Relay por internet)">
            <ol className="list-decimal list-inside space-y-1">
              ...
              <li>Escribe la <span className="text-fg">URL del relay</span> (ej. <span className="font-mono">https://sync.tudominio.com</span>).</li>
              <li>Escribe el <span className="text-fg">secret</span>: debe ser <span className="font-medium">exactamente el mismo</span> en todos los equipos y en el relay.</li>
              <li>Pulsa <span className="text-fg">Probar conexion</span>: debe decir &quot;Conexión exitosa&quot;.</li>
              <li>Pulsa <span className="text-fg">Guardar</span>.</li>
          ...
          <GuideSection title="3. Desplegar tu propio relay (opcional, para avanzados)">
            ...
            <pre className="mt-2 rounded-md bg-canvas p-3 text-xs text-fg-muted overflow-x-auto">
...
SYNC_SECRET="tu-secreto" PORT=8099 node server.js`}
      ...
      <Dialog open={tsDialogOpen} onOpenChange={(o) => { if (!tsBusy) setTsDialogOpen(o); }}>
        <DialogContent className="flex max-h-[85vh] flex-col border-line bg-surface-2 sm:max-w-md">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            ...
            {tsProgress.length > 0 && (
              <div className="rounded-md border border-line bg-canvas p-3">
                {tsProgress.map((m, i) => (
                  <p key={i} className="text-xs text-fg-muted font-mono">{m}</p>
                ))}
              </div>
            )}
            {tsDone && (
              <p className={cn('text-sm', tsDone.startsWith('Error') ? 'text-red-400' : 'text-brand')}>{tsDone}</p>
            )}
          </div>
  ...
  return (
    <div className="rounded-md border border-line bg-surface/40">
      <button
        ...
        className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-medium text-fg hover:bg-surface-2/50 transition-colors"
      >
        {title}
