    ...
    <div className="relative -m-4 flex min-h-[calc(100vh-4rem)] flex-col gap-0 overflow-hidden lg:-m-6 lg:h-[calc(100vh-4rem)]">
      ...
      <div className="pointer-events-none absolute inset-x-0 top-3 z-30 flex justify-center px-4 lg:top-5">
        <div className="pointer-events-auto w-full max-w-2xl">
          {/* Island bar */}
          <div className="flex items-center gap-1.5 rounded-2xl border border-line-strong/80 bg-surface-2/95 p-1.5 shadow-2xl shadow-black/50 backdrop-blur">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="h-10 border-line bg-surface/70 pl-9 pr-3 text-fg placeholder:text-fg-subtle focus-visible:ring-brand"
              />
            </div>
            {searchTerm && (
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-line text-fg-muted hover:bg-line-strong hover:text-fg transition-colors"
            ...
            <Button
              ...
              variant="outline"
              size="icon"
              className="h-9 w-9 shrink-0 border-line-strong"
              title="Leer código de barras con la cámara"
              onClick={() => setScannerOpen(true)}
            >
              <ScanLine className="h-4 w-4" />
            </Button>
            <span className="hidden shrink-0 rounded bg-line px-2 py-1 text-xs text-fg-muted sm:inline">
              F2
            </span>
          </div>
          ...
          {searchTerm && (
            <div className="mt-2 overflow-hidden rounded-2xl border border-line bg-surface shadow-2xl shadow-black/50">
              {/* Department chips */}
              {departments.length > 0 && (
                <div className="flex gap-2 overflow-x-auto border-b border-surface-2 px-3 py-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                  <button
                    type="button"
                    ...
                    className={cn(
                      ...
                      selectedDepartmentId === null
                        ? 'border-brand-strong bg-brand-strong text-brand'
                        : 'border-line-strong bg-surface-2 text-fg-muted hover:bg-line'
                    )}
                  >
                    Todos
                  </button>
              ...
              {/* Results count */}
              <div className="px-3 pt-2 text-xs text-fg-subtle">
                  ...
                  <div className="space-y-2">
                    {Array.from({ length: 12 }).map((_, i) => (
                      <div key={i} className="flex items-center gap-3 rounded-lg border border-line bg-surface-2 px-3 py-2.5">
                        ...
                        <Skeleton className="h-5 w-1/6 bg-line" />
                        <Skeleton className="h-6 w-16 bg-line" />
                        <Skeleton className="h-8 w-20 bg-line" />
                      </div>
                    ))}
                  </div>
                ) : products.length === 0 ? (
                          ...
                          <div
                            ...
                            className={cn(
                              ...
                              soldOut
                                ...
                                : 'border-line bg-surface-2 hover:border-brand-strong/50 hover:bg-line'
                            )}
                          >
                            ...
                            <Button
                              ...
                              className="h-8 w-[84px] shrink-0 bg-brand-strong text-xs font-bold text-brand hover:bg-brand"
                            >
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col">
        {/* Cart Header */}
        <div className="flex items-center justify-between border-b border-line bg-surface-2/60 px-6 py-3">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          {count === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-fg-subtle">
              <ShoppingCart className="mb-2 h-20 w-20 text-line" />
            ...
            <div className="mx-auto w-full max-w-3xl space-y-1.5 px-6 py-4">
              {cart.map((item) => (
                <div
                  key={item.productId}
                  className="flex items-center gap-2 rounded-lg border border-line bg-surface/50 px-3 py-2"
                >
                  <div className="min-w-0 flex-1">
        ...
        {/* Bottom Section */}
        <div className="border-t border-line bg-surface-2/60 px-4 py-3 lg:px-6">
          <div className="mx-auto grid w-full max-w-5xl gap-4 lg:grid-cols-2">
            {/* Left column: customer + payment method */}
            <div className="space-y-3">
              {/* Customer loyalty section */}
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-fg-muted">Cliente (fidelidad)</label>
                {selectedCustomer ? (
                  <div className="flex items-center justify-between rounded-lg border border-brand-strong bg-brand/20 p-2">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-medium text-brand">{selectedCustomer.name}</p>
                      <p className="text-xs text-brand-strong">{selectedCustomer.tier} - {selectedCustomer.purchaseCount} visitas</p>
                    </div>
                    <Button variant="ghost" size="sm" onClick={() => setSelectedCustomer(null)} className="h-7 text-xs text-fg-muted hover:text-red-400">Quitar</Button>
                  </div>
                ) : (
                  <div className="space-y-1">
                    <div className="flex gap-1">
                      <Input
                        ...
                        className="h-8 text-xs border-line-strong bg-surface text-fg"
                      />
                      <Button variant="outline" size="sm" onClick={handleFingerprintScan} disabled={scanningFingerprint} className="h-8 border-line-strong text-fg-muted" title="Escanear huella (F5)">
                        <Fingerprint className={`h-4 w-4 ${scanningFingerprint ? 'animate-pulse text-brand' : ''}`} />
                      </Button>
                    </div>
                    {searchingCustomer && customerSearch.length >= 2 && (
                      <div className="px-3 py-1.5 text-xs text-fg-subtle">Buscando...</div>
                    )}
                    {customerResults.length > 0 && customerSearch.length >= 2 && (
                      <div className="max-h-28 overflow-y-auto rounded-lg border border-line bg-surface">
                        {customerResults.map((c) => (
                          <div key={c.id} onClick={() => { setSelectedCustomer(c); setCustomerSearch(''); setCustomerResults([]); }} className="cursor-pointer px-3 py-1.5 text-xs text-fg-muted hover:bg-line">
            ...
            <div className="flex flex-col justify-between gap-2">
              ...
              <div>
                <Button
                  ...
                  className="w-full bg-brand-strong py-6 text-lg font-bold text-white hover:bg-brand"
                >
                  Cobrar — {formatCurrency(finalTotal)}
                </Button>
                <p className="mt-1.5 text-center text-[11px] text-fg-subtle">
                  <span className="rounded bg-line px-1.5 py-0.5">F2</span> escanear · <span className="rounded bg-line px-1.5 py-0.5">F3</span> cobrar
                </p>
              </div>
            </div>
          </div>
      ...
      <Dialog open={checkoutOpen} onOpenChange={setCheckoutOpen}>
        <DialogContent className="flex max-h-[85vh] flex-col sm:max-w-md">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
            {selectedCustomer && (
              <div className="rounded-lg border border-brand-strong bg-brand/20 p-3">
              ...
              <div className="rounded-lg bg-surface-2/50 p-4">
                ...
                <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                  <Button
                    ...
                    className={cn(
                      'border-line-strong bg-surface text-xs text-fg hover:bg-line',
                      cashReceived === finalTotal && 'border-brand bg-brand/40 text-brand'
                    )}
                  >
                    Exacto
                  </Button>
                  {[20, 50, 100, 200, 500, 1000].map((bill) => (
                    <Button
                      ...
                      className={cn(
                        'border-line-strong bg-surface text-xs font-bold text-fg hover:bg-line',
                        cashReceived === bill && 'border-brand bg-brand/40 text-brand'
                      )}
                    >
                      ${bill}
                    </Button>
                ...
                <Input
                  ...
                  className="mt-2 h-9 border-line-strong bg-surface text-fg"
                />
                <div className="mt-3 flex items-center justify-between rounded-md bg-canvas/60 px-3 py-2">
                  <span className="text-sm text-fg-muted">Cambio</span>
                  <span
                    className={cn(
            ...
            <div className="rounded-lg bg-surface-2/50 p-4">
              ...
              <div className="flex justify-between text-base font-bold">
                ...
                <span className="text-brand">
                  {loyaltyDiscount > 0 ? (
                    <><span className="text-xs text-fg-subtle line-through mr-2">{formatCurrency(taxedTotal)}</span> {formatCurrency(finalTotal)}</>
                  ) : (
                    formatCurrency(taxedTotal)
                  )}
                </span>
      ...
      <Dialog open={receiptOpen} onOpenChange={setReceiptOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button onClick={() => lastSale && handlePrintSale(lastSale)} className="bg-brand-strong hover:bg-brand">
  ...
  return (
    <div className="rounded-lg bg-surface-2/50 p-4 text-sm">
      <div className="mb-3 text-center">
        <p className="font-bold text-fg">TICKET DE VENTA</p>
        <p className="text-xs text-fg-subtle">Venta #{sale.id}</p>
        <p className="text-xs text-fg-subtle">{date}</p>
      </div>
      <div className="space-y-1.5">
        {sale.items.map((item, i) => (
          <div key={i} className="flex items-start justify-between gap-2 text-fg">
      ...
      <div className="mt-3 space-y-1 border-t border-line pt-2 text-fg-muted">
