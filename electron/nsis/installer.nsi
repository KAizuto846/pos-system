...
!insertmacro MUI_PAGE_CUSTOM ShowConfigPage CreateConfigPageLeave
...
Function ShowConfigPage
    ...
    Pop $ServerMode_Client
    ${NSD_CreateRadioButton} 0 160u 100% 20u "Automatico (detecta automaticamente)"
    Pop $ServerMode_Auto

    ; Set default selection (Server)
    SendMessage $ServerMode ${BM_SETCHECK} 1 0
    ...
    ${If} $AutoStart == "1"
        SendMessage $AutoStart_Checkbox ${BM_SETCHECK} 1 0
    ${EndIf}

    nsDialogs::Show
FunctionEnd

Function CreateConfigPageLeave
    ...
    ${NSD_GetState} $ServerMode $0
    ${If} $0 == ${BST_CHECKED}
        StrCpy $ServerMode "server"
    ${Else}
        ${NSD_GetState} $ServerMode_Client $0
        ${If} $0 == ${BST_CHECKED}
            StrCpy $ServerMode "client"
        ${Else}
            StrCpy $ServerMode "auto"
        ${EndIf}
    ${EndIf}
...
Section "Instalacion Principal" SEC_MAIN
    ...
    ; Copy all application files from the build output (created by electron-builder --dir)
